...
RUN apt-get update -qq && \
    ...
    export MAJOR_VER=$(echo ${PAPERCUT_MF_VERSION} | cut -d '.' -f 1); \
    sudo -u papercut -H bash <(curl -L https://cdn.papercut.com/web/products/ng-mf/installers/mf/$MAJOR_VER.x/pcmf-setup-${PAPERCUT_MF_VERSION}.sh -o -) -v --non-interactive && \
