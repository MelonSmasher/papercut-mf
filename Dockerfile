...
RUN apt-get update -qq && \
    ...
    sudo -u papercut -H bash -c 'bash <(curl -L https://cdn.papercut.com/web/products/ng-mf/installers/mf/$(echo '${PAPERCUT_MF_VERSION}' | cut -d "." -f 1).x/pcmf-setup-'${PAPERCUT_MF_VERSION}'.sh -o -) -v --non-interactive' && \
